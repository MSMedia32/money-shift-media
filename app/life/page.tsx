...
async function getLifeArticles() {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'https://money-shift.jp'}/api/articles?category=LIFE&limit=20`, {
      cache: 'no-store'
    });
...
export default async function LifePage() {
  const articles = await getLifeArticles()
    ...
    <WSJLayout>
      ...
      <div className="border-b border-gray-300 pb-6 mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-black mb-2">Life</h1>
            <p className="text-sm text-gray-600">ライフスタイル、健康、文化の情報</p>
          </div>
      ...
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
        <div className="flex items-center space-x-6">
          <Link href="/life" className="text-sm font-medium text-black border-b-2 border-purple-600 pb-1">
            All Life
          </Link>
